...
//
// 设计要点：
//   - 输入来自 shared/contracts.EscortSummary（跨服务契约）；
//     scorer 不再自定义 Escort 类型，避免与 escort-service 字段漂移。
//   - 输出仍是 []Candidate（EscortID + Score），便于 pool 写入。
//   - 城市 / 时段 / 评分 / 距离 四个维度。
//   - 所有权重 / 阈值写死为常量；v2 可在 config 覆盖。
package scorer
...
// 入参 escorts 直接来自 contracts.EscortSummary（无内部类型转换）。
func Compute(o Order, escorts []contracts.EscortSummary) []Candidate {
...
func scoreOne(o Order, e contracts.EscortSummary) float64 {
	// 1. 城市：必须匹配；否则 0
	if o.City != "" && e.City != "" && o.City != e.City {
		return 0
	}
	// 2. 时段：escort 可服务区间必须包含 order.service_time
	if !e.AvailableFrom.IsZero() && !e.AvailableUntil.IsZero() && !o.ServiceTime.IsZero() {
		if o.ServiceTime.Before(e.AvailableFrom) || o.ServiceTime.After(e.AvailableUntil) {
			return 0
		}
	}
...
func estimateDistance(o Order, e contracts.EscortSummary) float64 {
