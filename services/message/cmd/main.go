...
func main() {
	...
	// 依赖健康检查（/readyz）。message 无 DB；kafka brokers 非空时注册 checker；
	// nil / 空 → skip + warn（manager 仍创建，fail-closed 路由仍挂载）。
	...
	if len(cfg.Kafka.Brokers) > 0 {
		...
		logger.L().Info("message-service: readyz registered checker: kafka-brokers",
			zap.Strings("brokers", cfg.Kafka.Brokers))
	} else {
		logger.L().Warn("message-service: kafka.brokers empty; /readyz will fail-closed (no dependencies registered)")
	}
