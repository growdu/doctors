...
func TestPublisher_NilWriterReturnsError(t *testing.T) {
	var p *Publisher
	err := p.PublishVirtualNumberAllocated(context.Background(), contracts.VirtualNumberAllocatedEvent{VirtualNumberID: 1})
	assert.Error(t, err)
	err = p.PublishVirtualNumberReleased(context.Background(), contracts.VirtualNumberReleasedEvent{VirtualNumberID: 1})
	assert.Error(t, err)
}
