// Package middleware 提供 order-service 专用中间件。
//
// Auth 用 Authorization: Bearer <jwt> 解析 token，写入 gin.Context。
// 与 auth-service 的中间件契约一致，方便上下游复用。
package middleware
