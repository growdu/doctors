// Package server 负责启动与停止 order-service HTTP 服务。
//
// 设计要点：
...
import (
	...
	"github.com/growdu/doctors/services/order/internal/router"
...
// New 创建 Server；addr 监听地址，h 是业务 handler，jwtSecret 用于鉴权，
...
func (s *Server) Run(ctx context.Context) error {
	...
	case <-ctx.Done():
		...
		if err := s.httpSrv.Shutdown(shutdownCtx); err != nil {
			return fmt.Errorf("order: shutdown: %w", err)
		}
		s.runShutdownHooks()
		return nil
	}
